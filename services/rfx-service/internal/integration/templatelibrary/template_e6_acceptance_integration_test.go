...
func getTemplateVersionQuestionnaireHTTPWithActor(t *testing.T, env *testEnv, cfg config.Config, actor domain.ActorContext, templateID, versionID uuid.UUID) *httptest.ResponseRecorder {
	...
	router := httpserver.NewRouter(log, env.pool, cfg, env.rfxSvc, env.qSvc, env.versionSvc, env.templateSvc, env.templateQSvc, env.cloneSvc, env.crSvc, nil, nil, nil, env.scoreModelSvc, nil, nil, nil, nil)
