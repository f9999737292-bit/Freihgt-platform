...
func TestE7P2INT42RouteServiceGatewayOpenAPIParity(t *testing.T) {
	...
	for _, route := range routes {
		...
		case "preview_buyer_new_rfx_event_xlsx_create":
			...
			if route.RBACPolicy != "PolicyBuyerManage" || route.SuccessStatus != http.StatusOK {
				t.Fatalf("unexpected create preview contract: %+v", route)
			}
		case "commit_buyer_new_rfx_event_xlsx_create":
			if route.OpenAPIOperationID != "post_commit_buyer_new_rfx_event_xlsx_create" {
				t.Fatalf("unexpected operationId=%q", route.OpenAPIOperationID)
			}
			if !route.IdempotencyRequired || route.SuccessStatus != http.StatusCreated {
				t.Fatalf("unexpected create commit contract: %+v", route)
			}
		default:
