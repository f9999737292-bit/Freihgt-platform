...
func (m *mockFormTemplateRepo) ListActivePublished(ctx context.Context, tenantID uuid.UUID, entityType string, code string) ([]domain.FormTemplateSummary, error) {
	m.lastTenant = tenantID
	m.lastEntity = entityType
	m.lastActiveCode = code
	return m.activeItems, m.activeErr
}
...
func TestListActivePublishedRequiresEntityType(t *testing.T) {
	svc := NewFormTemplateService(&mockFormTemplateRepo{})
	_, err := svc.ListActivePublished(context.Background(), uuid.New(), "", "")
	if err == nil {
		t.Fatal("expected validation error")
	}
	var appErr *apperrors.AppError
...
func TestListActivePublishedFiltersByTenantAndCode(t *testing.T) {
	tenantID := uuid.MustParse("74519f22-ff9b-4a8b-8fff-a958c689682f")
	repo := &mockFormTemplateRepo{
		activeItems: []domain.FormTemplateSummary{
			{ID: uuid.New(), TenantID: tenantID, EntityType: "TRANSPORT_ORDER", Code: "transport_order_default", Status: domain.PublishedStatus, Version: 2, IsActive: true},
		},
	}
	svc := NewFormTemplateService(repo)
	items, err := svc.ListActivePublished(context.Background(), tenantID, "TRANSPORT_ORDER", "transport_order_default")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(items) != 1 || items[0].Version != 2 {
		t.Fatalf("expected active version 2, got %+v", items)
	}
	if repo.lastTenant != tenantID || repo.lastActiveCode != "transport_order_default" {
		t.Fatalf("tenant/code not forwarded to repository")
	}
}
