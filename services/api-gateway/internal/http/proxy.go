...
func NewProxyHandler(cfg config.Config) (*ProxyHandler, error) {
	routeDefs := []struct {
		...
		{"/api/v1/rfx-templates", "rfx-service", cfg.Services.RFX},
		...
		{"/api/v1/payment-allocations", "payment-service", cfg.Services.Payment},
...
func downstreamServices(cfg config.Config) []downstreamService {
	return []downstreamService{
		...
		{name: "freight-cost-service", url: cfg.Services.FreightCost},
	}
}
