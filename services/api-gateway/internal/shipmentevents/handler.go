...
func (h *Handler) Events(w http.ResponseWriter, r *http.Request) {
	...
	shipmentID, err := ValidateShipmentID(chi.URLParam(r, "shipmentId"))
	if err != nil {
		respond.Error(w, err)
		return
	}
	...
	response, err := h.service.GetEvents(r.Context(), reqCtx, shipmentID, query)
	if err != nil {
		respond.Error(w, err)
		return
	}
	...
	h.log.Info("shipment_events_completed",
		slog.String("request_id", requestIDFromRequest(r)),
		slog.String("tenant_id", reqCtx.TenantID),
		slog.String("shipment_id", shipmentID),
		slog.Int64("duration_ms", time.Since(started).Milliseconds()),
		slog.Int("events_count", len(response.Timeline.Items)),
		slog.Bool("partial", response.DataFreshness.Partial),
	)
}
...
func (h *Handler) ensureAccess(r *http.Request, reqCtx RequestContext) error {
	...
	if !CanViewShipmentEvents(roles) {
		return apperrors.Forbidden("shipment event history access denied")
	}
	return nil
}
