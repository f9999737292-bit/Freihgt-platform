...
GO_SERVICES := api-gateway identity-service company-service localization-service \
	transport-order-service shipment-service rfx-service document-service billing-register-service payment-service contract-rate-service low-code-service
...
.PHONY: help env-init dev-up dev-down dev-restart dev-logs ps db-shell db-check \
	...
	run-document-service run-billing-register-service run-payment-service run-contract-rate-service run-low-code-service \
	...
	system-test-design-check system-test-preflight system-test-smoke system-test-golden-skeleton staging-acceptance-pack system-test-wave1-security system-test-data-reset \
...
run-contract-rate-service:
	@cd services/contract-rate-service && go run ./cmd/server

test-contract-rate-service:
