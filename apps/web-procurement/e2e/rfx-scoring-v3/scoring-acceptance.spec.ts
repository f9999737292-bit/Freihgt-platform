...
test.describe("RFx v3.0D scoring browser acceptance", () => {
  ...
  test("studio publish + evaluation scores + legacy regression", async ({
    page,
    request,
  }) => {
    ...
    await expect(codeInputs).toHaveCount(2, { timeout: 30_000 });
    ...
    await expect(bindings.nth(1).locator("option")).toHaveCount(3, {
      timeout: 120_000,
    });
    await bindings.nth(0).scrollIntoViewIfNeeded();
    await bindings.nth(0).selectOption("ADR_AVAILABLE");
    await expect(bindings.nth(0)).toHaveValue("ADR_AVAILABLE");
    await bindings.nth(1).selectOption("FLEET_COUNT");
    await expect(bindings.nth(1)).toHaveValue("FLEET_COUNT");
    ...
    await expect(page.getByTestId("scoring-published-lock")).toBeVisible({
      timeout: 60_000,
    });
    await expect(page.getByTestId("scoring-model-status")).toContainText(
      /Published|Опубликована|已发布/i,
    );
    ...
    const evalApi = await request.get(
      `${gatewayURL}/api/v1/rfx-events/${eventId}/responses`,
    ...
    const responseIds = evalPayload.items
      .slice(0, 2)
      .map((item: { id: string }) => item.id);
    await assertBrowserScoresApi(page, responseIds);
    await page.goto(`${procurementURL}${evaluationPath()}`, {
      waitUntil: "domcontentloaded",
    });
    await expect(page.getByRole("status")).toHaveCount(0, { timeout: 120_000 });
    await expect(
      page
        .getByTestId("evaluation-comparison-table")
        .or(page.getByTestId("evaluation-responses-empty")),
    ).toBeVisible({ timeout: 30_000 });
    await expect(page.getByTestId("evaluation-comparison-table")).toBeVisible({
      timeout: 5_000,
    });
    await expect(
      page.getByTestId("legacy-commercial-score").first(),
    ).toBeVisible({ timeout: 60_000 });

    const v3Cells = page.getByTestId("v3-questionnaire-score");
    await expect(v3Cells).toHaveCount(2, { timeout: 120_000 });
    await expect
      .poll(async () => v3Cells.allTextContents(), { timeout: 120_000 })
      .toEqual(
    ...
    await expect(page.getByTestId("v3-knockout-badge")).toHaveCount(1, {
      timeout: 60_000,
    });

    await page.getByTestId("v3-explain-button").nth(1).click();
    await expect(page.getByTestId("v3-explanation-panel")).toBeVisible({
      timeout: 30_000,
    });
    await expect(page.getByTestId("v3-explanation-row").first()).toBeVisible();

    await page.goto(`${procurementURL}${evaluationPath(legacyEventId)}`, {
      waitUntil: "domcontentloaded",
    });
