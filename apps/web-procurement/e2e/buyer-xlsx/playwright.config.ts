...
if (required) {
  ...
  if (process.env.BROWSER_E2E_EXCEL_FLAG !== '1') {
    throw new Error('BROWSER_E2E_EXCEL_FLAG=1 is required for the buyer XLSX browser gate')
  }
}

if (!webURL) {
  throw new Error('BROWSER_E2E_WEB_URL is required for buyer XLSX Playwright; silent skip is not allowed')
}
...
export default defineConfig({
  ...
  projects: [
    { name: 'ui-stub', testMatch: 'buyer-xlsx-acceptance.spec.ts' },
    { name: 'live-stack', testMatch: 'buyer-xlsx-live.spec.ts' },
  ],
})
