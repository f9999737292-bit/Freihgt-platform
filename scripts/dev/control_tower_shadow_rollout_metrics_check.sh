...
resolve_jwt_token() {
  ...
  if [[ -z "${DEV_ADMIN_EMAIL}" || -z "${DEV_ADMIN_PASSWORD}" ]]; then
    fail "JWT_TOKEN or DEV_ADMIN_EMAIL+DEV_ADMIN_PASSWORD required for metric delta check"
  fi
...
if [[ "${REQUIRE_METRIC_DELTA}" == "true" ]]; then
  ...
  awk -v before="${before_cmp}" -v after="${after_cmp}" 'BEGIN { exit(after > before ? 0 : 1) }' \
    || fail "shadow comparison metric did not increase after summary request"
fi
