...
Waves are a review sequence. Completing discovery does not start wave I1. S1 is the only product change on this branch. I1–I4 stay `NOT_AUTHORIZED` and do not change product code, migrations, OpenAPI, or CI.
...
W0 remains accepted discovery. This branch implements S1 only. The controller accepted that remediation. I1–I4 remain `NOT_AUTHORIZED`.

```text
DOCUMENT_READ_TENANT_ISOLATION_REMEDIATION_REQUIRED
```

S1 is that remediation. Controller verdict `ACCEPT_EDO_0_3_S1` records product HEAD `dff9eacf04e0fd8c0e381d8716cf5a4039c90ff1`, pull request #164, and CI run `35991273506`. `TestDocumentReadTenantIsolation` passed. Document and signing-session reads are tenant-scoped. Foreign, missing, and deleted documents share one `404`. A missing trusted tenant returns `401`. OpenAPI and schema were not changed. Blocking findings are absent. I1, I2, I3, and I4 are not started. Legal verification stays open.
...
S1 has controller verdict `ACCEPT_EDO_0_3_S1`. I1 and later waves stay `NOT_AUTHORIZED` until a separate controller review authorizes that wave.
