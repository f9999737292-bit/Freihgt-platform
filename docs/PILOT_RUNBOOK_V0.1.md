...
Companion to `PILOT_OPERATIONAL_READINESS_AND_HANDOFF_V0.1.md` and `PILOT_LAUNCH_BLOCKER_CLOSURE_V0.1.md`. **Read-only reference — do not execute destructive steps without explicit authorized ops approval.**
...
**Rollback references:** `docs/BINTRANS_DEDICATED_CONTROL_TOWER_STAGING_RUNBOOK.md`, `docs/PILOT_RELEASE_MANIFEST_V0.1.md`, digest-pinned images in protected `staging.env`.

**Pinned Pilot release:** `CT-PILOT-2026-08-14-b75eb3d` @ git `b75eb3d` (see manifest).
...
**Restore drill status (2026-08-14 remediation):** `PASS` — fresh backup `freight_platform_20260814T134849Z.dump` (222315 bytes) validated and restored to disposable PostgreSQL. **Do not use** legacy backup `freight_platform_20260811T083942Z.dump` (913 bytes, empty).

**Backup scripts:** `scripts/ops/bintrans_ct_staging/bintrans_ct_staging_backup.sh` + `scripts/ops/validate_postgres_backup.sh` (fail-closed content validation).

**Safe drill script (disposable target only):** `scripts/ops/pilot_restore_drill_isolated.sh`

---

## Alerting (Interim Manual — Not Active)

Automated Alertmanager is **not deployed**. Until owners/channels assigned:

- See `docs/PILOT_INTERIM_ALERTING_V0.1.md`
- Monitoring: Prometheus + Grafana (localhost on VM) + health endpoints
- **Status:** `ALERT_ROUTING=BLOCKED` until P1/P2 owners + escalation channel defined

---

## On-Call Ownership

See `docs/PILOT_ON_CALL_ASSIGNMENT_V0.1.md`. **Status:** `ON_CALL_OWNERSHIP=BLOCKED` — most roles TBD.
