...
**Current status:** **SELECTEL_SSH_SG_RESTRICTION_EXECUTION_BLOCKED_PENDING_OPERATOR_INPUT** — STG-LIM-003 **open**; baseline API health **PASS**.
...
| W3-FB-SELECTEL-SSH-SG-RESTRICTION-EXEC-001 | 2026-07-11 | — | CROSS_ENTITY | TO/SH/BR demos | selectel SSH security group restriction execution evidence | P0 | Execution evidence capture attempted — blocked pending operator approval and Selectel SG change; baseline API health 200 | COMPLETED | — | Selectel SSH SG restriction re-run after operator execution | SELECTEL_SSH_SG_RESTRICTION_EXECUTION_BLOCKED_PENDING_OPERATOR_INPUT — stg_lim=STG-LIM-003, production_ready_claimed=no, ssh_executed=no, secrets_captured=no |
