...
**C2 merge:** PR #134 @ `66241e67d58c411efb9fa425902e459df65772fc` (2026-09-14)
**C3 merge:** PR #135 @ `e3341f0594cb749e5bb27d70053a495838eed3c8` (2026-09-14)
...
CARRIER_XLSX_OVERALL_STATUS=IMPLEMENTED_ACCEPTED
C1_EXPORT_TEST_IDS=E7P2-INT-71..79
C2_PREVIEW_TEST_IDS=E7P2-INT-80..90
C3_COMMIT_TEST_IDS=E7P2-INT-91..119
CARRIER_XLSX_SCHEMA=BINTRANS_RFX_CARRIER_XLSX_V1
...
CONTROLLER_VERDICT=ACCEPT_C3
REVIEWED_PRODUCT_HEAD=5326ff8c6d65531188f24de2915d848f0402b2dc
C2_MERGE_SHA=66241e67d58c411efb9fa425902e459df65772fc
C3_MERGE_SHA=e3341f0594cb749e5bb27d70053a495838eed3c8
CARRIER_XLSX_EXPORT_PROPOSED=YES
CARRIER_XLSX_PREVIEW_PROPOSED=YES
CARRIER_XLSX_COMMIT_PROPOSED=YES
CARRIER_XLSX_COMMIT_RESULT=DRAFT_ONLY
...
MATRIX_GUARD_NAME=TestE7P2CarrierXlsxMatrixIDsCompleteAndUnique
NEXT_TEST_ID_AFTER_CARRIER_XLSX=E7P2-INT-120
NEXT_ACTION=E7_PHASE2_ERP_API_ARCHITECTURE
...
## 22. C2 preview acceptance record (PR #134)

### Merge evidence

```
PR134_STATE=MERGED
PR134_HEAD=4a04e7ecb4c1f8a5b8373739f740489d6ec6b88c
PR134_BASE=501f6b4280c9b2aac813b406c9340e568ee1b86b
PR134_MERGE_SHA=66241e67d58c411efb9fa425902e459df65772fc
PR134_MERGED_AT=2026-09-14T16:05:52Z
PR134_CI_RUN_ID=34864516651
PR134_CI_CONCLUSION=success
C2_PRODUCT_HEAD=5b2fa47aa47a8275ac444edde86551c10f0d87fe
CONTROLLER_ACCEPTANCE=YES
CONTROLLER_VERDICT=ACCEPT_C2
C2_STATUS=MERGED_ACCEPTED
```

### Accepted C2 properties

| Property | Status |
|---|---|
| Carrier import preview parser (`BINTRANS_RFX_CARRIER_XLSX_V1`) | **YES** |
| Valid-only analysis persistence (`PREVIEWED`) | **YES** |
| Preview HTTP route (no response/answer/offer writes) | **YES** |
| Service route `POST …/xlsx-import/preview` | **YES** |
| Gateway route `POST /api/v1/rfx-events/{id}/carrier-responses/{response_id}/xlsx-import/preview` | **YES** |
| E7P2-INT-80..90 PASS (PostgreSQL 16 runtime) | **YES** |
| C1 export regression PASS | **YES** |
| Buyer XLSX regression PASS | **YES** |
| Migration 000074 absent | **YES** |
| Carrier Commit route absent at C2 merge | **YES** |
| Auto-submit absent | **YES** |

### Deferred / open findings (non-blocking at C2 merge)
...
## 23. C3 commit acceptance record (PR #135)

### Merge evidence

```
PR135_STATE=MERGED
PR135_HEAD=b48af2ffd9f0d5b7dc67f1cbf74f91f1264ae8ca
PR135_BASE=66241e67d58c411efb9fa425902e459df65772fc
PR135_MERGE_SHA=e3341f0594cb749e5bb27d70053a495838eed3c8
PR135_MERGED_AT=2026-09-14T19:19:00Z
PR135_CI_RUN_ID=34884705304
PR135_CI_CONCLUSION=success
C3_PRODUCT_HEAD=5326ff8c6d65531188f24de2915d848f0402b2dc
CONTROLLER_ACCEPTANCE=YES
CONTROLLER_VERDICT=ACCEPT_C3
C3_STATUS=MERGED_ACCEPTED
```

### Accepted C3 properties

| Property | Status |
|---|---|
| Atomic commit from stored preview analysis to DRAFT response | **YES** |
| Body `{ "analysis_id" }` only; mandatory `Idempotency-Key` | **YES** |
| Answers patch/reconcile; offer lines replace-all | **YES** |
| `PREVIEWED` → `CONSUMED`; stale baseline rejection | **YES** |
| No auto-submit; `submitted_at` unchanged | **YES** |
| Late permission validated but not consumed | **YES** |
| Service route `POST …/xlsx-import/commit` | **YES** |
| Gateway route `POST /api/v1/rfx-events/{id}/carrier-responses/{response_id}/xlsx-import/commit` | **YES** |
| operationId `post_commit_carrier_rfx_response_xlsx_import` | **YES** |
| E7P2-INT-91..119 PASS (PostgreSQL 16 runtime) | **YES** |
| C1/C2/Buyer regression PASS | **YES** |
| Six E7 Excel Exchange routes on manifest | **YES** |
| Migration 000074 absent | **YES** |

### Logical commits (PR #135 product head)

| # | SHA | Message |
|---|---|---|
| 1 | `018f38f2` | `feat(rfx): apply Carrier XLSX import analyses` |
| 2 | `98ba5780` | `feat(rfx): expose Carrier XLSX import commit API` |
| 3 | `00808394` | `test(rfx): cover Carrier XLSX C3 commit acceptance` |
| 4 | `8103f1fe` | `docs(rfx): record Carrier XLSX C3 implementation` |
| 5 | `5326ff8c` | `docs(openapi): define Carrier XLSX import commit contract` |

Docs acceptance commit `b48af2ff` (`docs(rfx): record Carrier XLSX C3 acceptance`) is on PR head after product head.

### Open findings (non-blocking at C3 merge)
