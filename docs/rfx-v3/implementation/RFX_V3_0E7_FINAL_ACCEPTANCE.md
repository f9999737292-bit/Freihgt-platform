...
E7_PHASE_1_CONTROLLER_VERDICT=ACCEPT
...
E7_BROWSER_ACCEPTANCE=IMPLEMENTED_ACCEPTED
E7_BROWSER_ACCEPTANCE_STATUS=IMPLEMENTED_ACCEPTED
CONTROLLER_VERDICT=ACCEPT_E7_BROWSER_ACCEPTANCE
...
E7_PHASE_2=IMPLEMENTATION_IN_PROGRESS
FRONTEND_PHASE2_STATUS=IMPLEMENTATION_IN_PROGRESS
...
NEXT_ACTION=PLAN_F5_TEMPLATE_ACQUISITION_OR_TRAINING
```

---

## 12. E7 browser acceptance (accepted)

| Marker | Value |
|---|---|
| `E7_BROWSER_ACCEPTANCE_STATUS` | `IMPLEMENTED_ACCEPTED` |
| `CONTROLLER_VERDICT` | `ACCEPT_E7_BROWSER_ACCEPTANCE` |
| Accepted product HEAD | `fd5b615ff408f4d6e9b471cf687211d3ec7fcf80` |
| Accepted product CI | `35644222482` |
| E7 result | 13 passed / 0 failed / 0 skipped / retries 0 |
| Main-chain | one event and one response through Award |

Scoped contract:

- New Studio draft defaults `questionnaire_enabled=false`.
- Successful `PublishQuestionnaire` enables and publishes atomically.
- Failed publish leaves `questionnaire_enabled=false`.
- GET workspace unbound → 422 (`UNPROCESSABLE_ENTITY`, `details.field=rfx_version_id`).
- Carrier start pinning keeps the same response ID and offer.

Findings: `F-QE-DEFAULT=FIXED`, `F-OAPI-422=FIXED`, `F-LIVE-OFFER-PIN=FIXED`, `F-INPUT-NUMBER=NOTE`, `F-QE-VALIDATE-PUBLISH=NOTE`.

Still open after this gate: `FRONTEND_PHASE2_STATUS=IMPLEMENTATION_IN_PROGRESS`. F5 Create-from-XLSX backend W1 is `IMPLEMENTED_ACCEPTED` (product HEAD `1f5deadaa4e81e352c041a5b97318b1a640ac48f`, CI `35695381395`, verdict `ACCEPT_F5_BACKEND_W1`). F5 frontend upload-only is `IMPLEMENTED_ACCEPTED` (product HEAD `2acb7c0e27381c69707bde409447452a6e54d2f7`, CI `35731621656`, verdict `ACCEPT_F5_FRONTEND_UPLOAD_ONLY`). F5 overall remains `IMPLEMENTATION_IN_PROGRESS`. F5 blank-template acquisition and training are `NOT_STARTED`. ERP browser client is forbidden/not started. TMS is out of scope. Award→Transport Order is not part of the E7 browser gate.
